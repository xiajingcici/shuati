...
public class ThreeSumClosest {
	//brutal force O(n^3)
	public int threeSumClosest1(int[] sum,int target){
	...
	//optimize
	public int threeSumClosest(int[] sum,int target){
		int res=sum[0]+sum[1]+sum[2];
		int mindiff=Math.abs(res-target);
		Arrays.sort(sum);
		for(int i=0;i<sum.length-2;i++){
